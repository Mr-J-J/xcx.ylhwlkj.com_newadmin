...
class RsWithDraw extends Model
{
    protected $table ="rs_store_withdraw";

    public static function getDrawList($store_id,$pageNum = 10){
        $list = self::select('id','store_id','title','money','draw_account','account_name','created_at','state')
     ...
     * @return void
     */
    static function addDraw(RsStores $storeInfo,$type,$money = 0){
               
        // $store = RsStores::where('id',$storeInfo->store_id)->first();
        $draw = new RsWithDraw;
        $draw->store_id = $storeInfo->id;
        ...
        $draw->draw_account = ($type == 1)?'':$storeInfo->alipay_account;
    ...
    public function store(){
        return $this->hasOne('App\CardModels\RsStores','id','store_id');
    }
