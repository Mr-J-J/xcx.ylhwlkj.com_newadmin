...
class UserOrder extends Model
{
    ...
    public static function createOrder(array $data,TicketUser $user){
        ...
        $paiqidInfo = Api\Schedules::where('show_index',$paiqi_id)->first();
        if(empty($paiqidInfo)){
        ...
        if($stopTime){
            $showtime = strtotime("- {$stopTime} minute",$paiqidInfo->show_time);
            if($showtime <= time()){
        ...
        $order->halls = $paiqidInfo->hall_name;
        $order->cinema_id = $paiqidInfo->cinema->id;
        $order->cinemas = $paiqidInfo->cinema->cinema_name;
        $order->citys = Api\City::where('id',$paiqidInfo->cinema->city_code)->value('name');
        $order->brand_id = $paiqidInfo->cinema->brand_id;
        $order->seat_areas = $section_id;

        $originalPrice = round($paiqidInfo->getOriginal('price') / 100,2);
        $order->ticket_count   =  count($seatsArray);
        $order->market_price = $originalPrice; //快速购票价格
        $order->discount_price = $originalPrice - $paiqidInfo->price; //优惠金额
        $order->use_card = 0;
        $order->amount = $paiqidInfo->price;
        //特惠
        ...
        if($order->buy_type == 1){
            ...
            $order->amount = $paiqidInfo->local_price;  //特惠购票价格
        }
        ...
        if(!empty($com_id)){
            ...
            if($cinemaBrand){
                ...
                if($order->buy_type == 1){
                    $order->discount_price = $discountPrice = $cinemaBrand->calcDiscountMoney($paiqidInfo->api_price);
                }

                if($discountPrice && $userWalletBalance >= $discountPrice){
                    $order->use_card = 1;
                    $order->amount = $paiqidInfo->api_price - $discountPrice;
                }else{
                    $order->discount_price = $discountPrice = 0;
                    $order->amount = $paiqidInfo->api_price;
                }
            }
        ...
        $order->vprice = round($paiqidInfo->vprice/100,2);
        $order->movie_name = $paiqidInfo->film->show_name;
        $order->movie_image = $paiqidInfo->film->poster;
        $order->show_time = $paiqidInfo->show_time;
        $order->show_version = $paiqidInfo->show_version;
        $order->close_time = $paiqidInfo->close_time;
        // try {
...
//            UserWallet::walletKouFee($user,$order);
     ...
     * @return void
     */
    public function redirectOutTicketNEW(){
        $order = $this;
        if(empty($order->api_lock_sid)){
            return false;
        }
        try {
            $vprice = round($order->vprice * $order->ticket_count,2);
            $orderAmount = $vprice?:round($order->market_price / 100,2);
            $apiOrderModel = new ApiOrders;
...
//            \App\Support\MovieApiFactory::api('jufubao')->payOrder($order->api_lock_sid,$order->order_no);
            ...
            if($res['result']['status']!=0){
                ...
                \App\Jobs\Wangpiao\OutTicketJob::dispatch($api_order)->delay(2);
            }
//            logger($res);
        } catch (\Throwable $th) {
            logger('订单号'.$order->getOrderNo().',新接口直接出票：'.$th->getMessage());
        }
    }
    /**
     * 网票网直接出票
    ...
    public function redirectOutTicket(){
        ...
        if($setting['jiekoufang']['content']==1){
            ...
            return $this->redirectOutTicketNEW();
        }
