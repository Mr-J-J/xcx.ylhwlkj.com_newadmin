...
class MallController extends Controller
{
    ...
    public function productDetail(Request $request){
        ...
        $product->storage = 0;
        $product->sub_title = $product->sub_title?:'';
...
//        从数据表mall_store中获取商家信息
        $store = M\Stores::getStoreInfo($product->store_id);
        $tickuser = D\TicketUser::where('id',$store->user_id)->first();
        $product->store = $store;
        $product->store->tickuser = $tickuser;
    ...
    public function productList(Request $request){
       ...
       if(empty($storeIds)){
        ...
        $list = collect(['category_id'=>$cid,'category_title'=>$title,'category_type'=>$category->type??''])->merge($list);
       return $this->success('',$list);
    }
    ...
    protected function getCityId($city_code){
        ...
        $newCode = $this->correctCity($this->cityCode);
        if($newCode){
            $this->cityCode = $newCode;
        }
    }

    protected function correctCity($city_code){
        $array = array(
            '110000'=>'110100'
        );
        return $array[$city_code]??'';
    }
