...
class UsersController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        if($comId){
            $grid->model()->where('com_id','>',0);
        }else{
            $grid->model()->where('com_id',0);
        }
        $grid->tools(function($tool) use ($inviterId,$comId){
            
            if($inviterId){
                $tool->append("<a class='btn btn-sm btn-primary' href='/admin/users'>查看所有用户</a>");
            }
            $tool->append("<a href='/admin/users' class='btn btn-sm btn-".($comId == 0?'warning':'default')."'>影旅汇小程序</a>");
            $tool->append("<a href='/admin/users?com_id=1' class='btn btn-sm btn-".($comId == 1?'warning':'default')."'>分销小程序</a>");
        });
        ...
        $grid->column('nickname', __('昵称'))->display(function($nickname){
            
            if($this->com_id){
                return "<span class='label label-success'>分销</span> ".$nickname;
            }
            return $nickname;
        });
        ...
        $grid->column('total_balance', '累计佣金')->link(function(){
            return '/admin/user-commision?mobile='.$this->mobile;
        },'');
