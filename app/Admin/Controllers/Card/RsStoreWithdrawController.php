...
class RsStoreWithdrawController extends AdminController
{
    /**
    ...
    {
        $grid = new Grid(new RsWithDraw());
        $grid->model()->latest();
        $grid->disableCreateButton();
        $grid->disableFilter();
        $grid->quickSearch(function($model,$query){            
            $list = RsStores::where('store_name','like',"%{$query}%")->paginate(20,['id']);
                $ids = array();
        ...
        $grid->actions(function($action){
            ...
            if($this->row->state  == 0){
                ...
                $action->add(new RsStoreWithdrawErr());
            }
        });
        ...
        $grid->column('store_id', __('商家名称'))->display(function(){
            ...
            return url("admin/rs-stores/{$this->store_id}/edit");
        },'');
    ...
    {
        $show = new Show(RsWithDraw::findOrFail($id));
