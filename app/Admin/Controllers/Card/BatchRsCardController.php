...
class BatchRsCardController extends AdminController
{
    /**
    ...
    protected function grid()
    {
        ...
        $grid->model()->where('type',2)->latest();
        $grid->actions(function($actions){
            $actions->disableEdit();
            $actions->disableDelete();
            $actions->disableView();
            $count = RsOlCard::where('batch_id',$this->row['id'])->where('state','<',2)->count();
            if($count){
                $actions->add(new BatchStartUse());
            }
        });
        $grid->column('title', '批次')->link(function(){
            return '/admin/rs-cards-list?batch_id='.$this->id;
        },'');
        $grid->column('store.store_name','分销商');
        $grid->column('product.title','卡类型');
        $grid->column('number', '生成数量');
        $grid->column('bind_number', '已激活数量')->display(function(){
            return RsOlCard::where('batch_id',$this->id)->where('state',10)->count();
        });
    ...
    protected function form()
    {
        ...
        $form->select('product_id','卡类型')->options($cardList)->rules('required',['required'=>'请选择卡片类型']);
        $form->text('title', '批次')->default('影旅卡- '.date('Y-m-d'));
        $form->number('number', '生成卡数量')->min(1)->max(6000)->default(1)->help('单次最多生成6000张卡');
        $form->hidden('type')->value(2);//影旅卡
        if($form->isCreating()){
            $form->saved(function($form){
                $form->model()->batchCreateRsCard();
            });
        }
        return $form;
    }
