...
use App\Admin\Actions\Pft\OrderOutTicket;
use App\Admin\Actions\Pft\OrderRefundTicket;
use Encore\Admin\Controllers\AdminController;

class PwTicketOrdersController extends AdminController
{
    /**
    ...
    protected function grid()
    {
        ...
        $grid->model()->whereIn('order_status',[21])->latest();
        // $grid->column('id', __('Id'));
        $grid->column('order_no', '订单号')->display(function($orderNo){
            return $orderNo."<br />" ."票付通订单号：<span style='color:#1087DD'>{$this->UUordernum}</span>";
        });
        $grid->column('user.nickname', '用户');
        $grid->column('UUltitle', '门票')->display(function($title){
            return $title . "<br />".$this->UUttitle;
        });
        $grid->column('UUplaytime','游玩日期')->display(function($UUplaytime){
            return date('Y-m-d',strtotime($UUplaytime)) . "<br />{$this->UUtnum}张";
        });;
        $grid->column('UUlprice', '单价')->display(function($UUlprice){
            return "￥".round($UUlprice /100,2);
        });
        // $grid->column('UUtotalmoney', __('UUtotalmoney'));
        $grid->column('order_amount', '订单金额')->display(function($order_amount){
            return "￥".round($order_amount /100,2);
        });;
        // $grid->column('transaction_id', __('Transaction id'));
        $grid->column('order_status', '订单状态')->display(function(){
            return $this->getStatusTxtWithLabel();
        });

        $grid->column('ticket_status', '出票状态')->width(200)->display(function(){
            $exceptionInfo = \App\UUModels\UUOrderException::where('order_id',$this->id)->where('state',0)->latest()->first();
            if(!empty($exceptionInfo)){        
                return "<a href='/admin/pw-exception-order?id={$exceptionInfo->id}'>{$exceptionInfo->errorinfo}</a>";
            }
            if(!empty($this->UUcode)){
                return '出票成功';
            }
            return '出票中';
        });
        ...
        $grid->filter(function($filter){
            ...
            $filter->equal('UUordernum','票付通订单号');
            $filter->equal('UUordertel','联系电话');
        ...
        $grid->actions(function($action){
            ...
            $action->add(new OrderOutTicket());
            if($action->row->order_status == 21){
                $action->add(new OrderRefund());
            }
        });
