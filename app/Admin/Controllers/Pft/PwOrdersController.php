...
class PwOrdersController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->column('UUplaytime','游玩日期')->display(function($UUplaytime){
            return date('Y-m-d',strtotime($UUplaytime));
        });;
        $grid->column('UUtnum', '门票数量');
        ...
        $grid->column('order_status', '订单状态')->display(function($order_amount){
            
            if($this->refund_status){
                // $refundOrder = \App\UUModels\UUOrderRefund::getOrderByOrderNo($this->order_no);
                // // dd($refundOrder);
                // if($refundOrder){
                //     $arr = ['退款中','退款成功', '退款失败'];
                //     return "<span class='label label-default'>".$arr[$refundOrder->state]."</span>";
                // }
            }
